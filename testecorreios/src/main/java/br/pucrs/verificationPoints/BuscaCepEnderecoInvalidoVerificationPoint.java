...
public class BuscaCepEnderecoInvalidoVerificationPoint {
	...
	public void checarBuscaCepEnderecoInvalido() throws InterruptedException{
		...
		if(this.driver.getPageSource().contains(expectedMessage)) {
			Report.log(Status.PASS, "O Teste Retornou A Mensagem Esperada \"DADOS NAO ENCONTRADOS\"",  Screenshot.capture(driver));
			pass = true;
		}	
		
		else {
			Report.log(Status.FAIL, "O Teste Não Retornou A Mensagem Esperada \"DADOS NAO ENCONTRADOS\"",  Screenshot.capture(driver));
		}
