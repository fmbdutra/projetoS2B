...
public class BuscaCepEndereco94820010VerificationPoint {
	...
	public void checarBuscaCepEndereco94820010() throws InterruptedException {
		...
		if(this.driver.getPageSource().contains(expectedMessage)) {
			Report.log(Status.PASS, "O Teste Retornou A Mensagem Esperada \"Travessa Ana Neri\"",  Screenshot.capture(driver));
			pass = true;
		}	
		else {
			Report.log(Status.FAIL, "O Teste Não Retornou A Mensagem Esperada \"Travessa Ana Neri\"",  Screenshot.capture(driver));
		}
