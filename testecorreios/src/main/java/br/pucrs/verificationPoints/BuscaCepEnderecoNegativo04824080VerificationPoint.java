...
public class BuscaCepEnderecoNegativo04824080VerificationPoint {
	...
	public void checarBuscaCepEnderecoNegativo04824080() throws InterruptedException {
		...
		if(this.driver.getPageSource().contains(expectedMessage)) {
			Report.log(Status.PASS, "O Teste Retornou A Mensagem Esperada \"Rua Raiz de Guiné\"",  Screenshot.capture(driver));
			pass = true;
		}	
		else {
			Report.log(Status.FAIL, "O Teste Não Retornou A Mensagem Esperada \"Rua Raiz de Guiné\"",  Screenshot.capture(driver));
		}
